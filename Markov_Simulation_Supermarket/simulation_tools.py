...
def arrival_time_from_hour_distribution(df, date):
    ...
    df_time = pd.to_timedelta(df_time['time']).copy()  #, unit='s'
...
def simulate_customers(df_entrance):
    """
    ...
    for id in df_entrance['customer_id']:
        df_customer = df_entrance.loc[df_entrance['customer_id']==id,:].copy()
        ...
        entrance_time = df_customer.loc[df_customer['customer_id']==id,'time'].iloc[0]
        #Number of minute to add to entry time to have leaving time
        minutes_to_add = new_customer.nb_state-1
        leaving_time = pd.to_datetime(df_entrance.loc[df_entrance['customer_id']==id,'time']) + pd.Timedelta(f'{minutes_to_add} min')
        #Getting leaving time in good format
        leaving_time = leaving_time.iloc[0]
        #Creating time range from arrival to leaving time
        minute_index = pd.date_range(entrance_time,leaving_time, freq="min")
